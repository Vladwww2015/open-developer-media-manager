    ...
    (function () {
        ...
        function defaultMediaMangerResponse(result){
            ...
            if (typeof result.data === 'object') {
                ...
                if (data.status) {
                    ...
                    developer.toastr.error(data.message);
                }
            }
        }
        ...
        document.querySelectorAll('.file-delete').forEach( el => {
            el.addEventListener("click",function () {
                ...
                Swal.fire({
                    title: "{{ __('developer.delete_confirm') }}",
                    type: "warning",
                    showCancelButton: true,
                    confirmButtonColor: "#DD6B55",
                    confirmButtonText: "{{ __('developer.confirm') }}",
                    showLoaderOnConfirm: true,
                    closeOnConfirm: false,
                    cancelButtonText: "{{ __('developer.cancel') }}",
                    preConfirm: function() {
                        return new Promise(function(resolve) {
                            ...
                            developer.ajax.request(url,obj,resolve);
                        });
                    }
        ...
        document.querySelector('.goto-url button').addEventListener('click',function () {
            ...
            developer.ajax.navigate('{{ $url['index'] }}?path=' + path);
        });
